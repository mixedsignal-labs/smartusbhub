...
def pytest_addoption(parser):
    """Add common stress-test CLI options."""
    group = parser.getgroup("smartusbhub")
    group.addoption("--stress-count", action="store", type=int, default=None,
                    help="Stress test iteration count")
    group.addoption("--stress-channels", action="store", default=None,
                    help="Stress test channels, e.g. 1 or 1,2")
    group.addoption("--stress-mode-interval", action="store", type=float, default=None,
                    help="Charge mode dwell interval in seconds before each switch")
    group.addoption("--stress-switch-delay", action="store", type=float, default=None,
                    help="Post-switch settle delay in seconds before verify")
    group.addoption("--stress-adb-check", action="store_true", default=False,
                    help="Enable adb online checks during stress tests")
    group.addoption("--stress-adb-serial", action="store", default=None,
                    help="Target adb serial for online check")
    group.addoption("--stress-adb-check-delay", action="store", type=float, default=None,
                    help="Delay before adb check after switch")
