...
def generate_stats_html(stats, op_names, supports_usb2,
...
def check_dataline_support(hub):
    ...
    try:
        ...
        if product_info:
            return product_info.get("enable_usb2_data_switch", False)
    except:
        pass
    return False
...
def test_stress_core_functions(hub, max_channels, request):
    ...
    # 检查是否支持数据线切换
    supports_usb2 = check_dataline_support(hub)
    if supports_usb2:
    ...
    dataline_str = "USB2数据线" if supports_usb2 else ""
    ...
    try:
        ...
        html_content = generate_stats_html(stats, op_names, supports_usb2,
                                          total_operations, success_count, failure_count,
                                          success_rate, elapsed_time, ops_per_sec,
