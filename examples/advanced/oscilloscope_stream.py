"""
@file oscilloscope_stream.py
@brief Real-time voltage & current oscilloscope — V3 stream protocol.
...
Run:
    python examples/advanced/oscilloscope_stream.py
"""
...
class StreamOscilloscopeApp(QtWidgets.QMainWindow):
    def __init__(self, hub: SmartUSBHub, *args, **kwargs):
        ...
        self.channels = list(hub.get_channels())
        ...
        print(f"[oscilloscope] Powering on channels {self.channels} before measurement")
        self.hub.set_channel_power(*self.channels, state=1)
        time.sleep(0.1)
